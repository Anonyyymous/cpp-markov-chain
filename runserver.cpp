...
std::map<std::string, NChain*> models;
...
std::string parse_punctuation(std::string prompt) {
    if(prompt == "")
        return "''";

    size_t pos = prompt.find("%");

    while (pos != std::string::npos && pos < prompt.size()-2) {
        try {
            int code = std::stoi(prompt.substr(pos+1,2), nullptr, 16);
...
std::string convert_to_json(std::string result, int status_code) {
...
HTTPResponse process_request(HTTPRequest request, bool debug) {
    ...
    if(!(std::filesystem::exists(target_model) && std::filesystem::path(target_model).extension() == ".jkc")) {
        ...
        if(models.count(target_model) == 0) {
            ...
            if(debug)
                std::cout << "model: " << target_model << " loaded" << std::endl;
            model->debug = debug;
...
/// @param config_path 
void make_config(std::string config_path) {
    // default values
    port = DEFAULT_PORT;
...
/// @param config_path 
void process_config(std::string config_path) {
    if(std::filesystem::exists(config_path)) {
        ...
        try{
            ...
            std::cout << "couldnt successfuly read config file... ";
